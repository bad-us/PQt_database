...
def gui_create_model(database):
    ...
    list_tbl.setHorizontalHeaderLabels(['Логин клиента', 'IP Адрес', 'Порт', 'Время подключения'])
...
def create_stat_model(database):
    ...
    list_row.setHorizontalHeaderLabels(['Логин клиента', 'Последний раз входил', 'Сообщений отправлено', 'Сообщений получено'])
...
def create_stat_login_history_model(database):
    ...
    list_row = QStandardItemModel()
    list_row.setHorizontalHeaderLabels(['Логин', 'Дата входа', 'ip адрес клиента', 'порт клиента'])
    for row in hist_list:
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        self.show_login_history_button = QAction('История коннектов клиентов', self)
        ...
        self.setWindowTitle('Сервер чатика-недоделки')
...
class LoginHistoryWindow(QDialog):
    ...
    def initUI(self):
        # настройки окна
        self.setWindowTitle('история входов клиентов')
        self.setFixedSize(600,700)
        self.setAttribute(Qt.WA_DeleteOnClose)
        # кнопка закрытия
        self.close_button = QPushButton('Закрыть', self)
